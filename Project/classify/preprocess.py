...
class ColumnSelector(BaseEstimator, TransformerMixin):
    # Function to analyse the columns to be selected from the DataFrame

    def __init__(self, columns):
        self.columns = columns # Columns to be selected from the DataFrame

        
    def fit(self, X, y=None):
        return self # it is not being used, just to be consisted with sklearn 

    
    def transform(self, X):
        assert isinstance(X, pd.DataFrame) # Checking if the input is a DataFrame

        try:
            return X[self.columns] # Selects the columns from the DataFrame
        except KeyError:
...
class TypeSelector(BaseEstimator, TransformerMixin):
    # Selects columns from a Pandas DataFrame based on their dtype
...
class CategoryTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, categorical_features):
        # Column names of Categorical data type that we want to transform
...
class ImportanceModelPreprocessor (BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_boolean = pd.DataFrame(data=boolean_transformed,
                                 ...
                                 index=X.index)
        df = pd.concat([X_numerical, X_boolean], axis=1)
        return df
...
def make_complete_preprocessor (numeric=True, categorical=False, boolean=True):
    transformer_list = []
    if numeric:
